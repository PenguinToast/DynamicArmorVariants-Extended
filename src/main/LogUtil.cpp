...
namespace LogUtil
{
	auto GetFormName(const RE::TESForm* a_form) -> std::string
	{
		...
		if (const auto* editorID = a_form->GetFormEditorID(); editorID && editorID[0] != '\0') {
			return editorID;
		}
	...
	auto DescribeArmorAddon(const RE::TESObjectARMA* a_armorAddon) -> std::string
	{
		...
		return GetFormName(a_armorAddon) + " [" + FormatFormID(a_armorAddon->GetFormID()) +
			"] slots=" + FormatFormID(static_cast<RE::FormID>(a_armorAddon->bipedModelData.bipedObjectSlots.underlying()));
	}
	...
	auto DescribeArmor(const RE::TESObjectARMO* a_armor) -> std::string
	{
		...
		return GetFormName(a_armor) + " [" + FormatFormID(a_armor->GetFormID()) +
			"] slots=" + FormatFormID(static_cast<RE::FormID>(a_armor->bipedModelData.bipedObjectSlots.underlying()));
	}
	...
	auto DescribeOverrideOption(ArmorVariant::OverrideOption a_option) -> std::string_view
	{
		switch (a_option) {
