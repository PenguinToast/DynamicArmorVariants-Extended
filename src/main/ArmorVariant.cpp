...
auto ArmorVariant::GetAddonList(const RE::TESObjectARMA *a_armorAddon) const
    -> const AddonList * {
...
auto ArmorVariant::WouldReplace(const RE::TESObjectARMA *a_armorAddon) const
    -> bool {
  return GetAddonList(a_armorAddon) != nullptr;
}

auto ArmorVariant::WouldReplaceAny(const RE::TESObjectARMO *a_armor) const
    -> bool {
  if (!a_armor) {
    return false;
  }
  ...
  for (auto &armorAddon : a_armor->armorAddons) {
    if (armorAddon && WouldReplace(armorAddon)) {
      return true;
    }
  }
